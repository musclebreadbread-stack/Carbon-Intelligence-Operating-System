  ...
  return [
    {
      ...
      blocks: [
        {
          kind: "text",
          text: "이 시스템은 환경변수가 하나도 없어도 실행됩니다. 이때는 '데모 모드'로 동작하며, 19개 대시보드 화면의 모든 수치는 내장 샘플 데이터를 실제 계산 엔진(Scope 1/2/3 산정, SBTi 경로, 시나리오, MACC, 크레딧, 공시 매핑, MRV)으로 계산한 값입니다. 저장(쓰기) 동작만 거부됩니다.",
        },
        {
          kind: "steps",
          items: [
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "전체 UI 한국어 번역 완료: 19개 대시보드 페이지의 모든 본문, 버튼, 폼 라벨, 탭, KPI, 차트 라벨이 딕셔너리 기반 한국어로 전환됨 (설정에서 영문 전환 가능)",
            "증빙 업로드 MIME/크기 제한: PDF, 이미지, Office 문서, CSV, ZIP만 허용, 최대 50MB",
            "ESG 공시 내보내기 완성도 100배 오류 수정 및 페이지 헤더에 다운로드 버튼 추가",
            "접근성: main-content ID, MobileDrawer 768px 이하 모바일 내비게이션, 사이드바 접기/펼치기 aria-label",
          ],
        },
      ],
      verification: [
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            [
              "한국어 UI",
              "완료. 전체 UI가 한국어를 기본값으로 사용하며, 설정에서 영문으로 전환할 수 있습니다. 딕셔너리 기반 i18n이 모든 페이지의 제목, 본문, 버튼, 폼, 탭, KPI, 차트 라벨에 적용되어 있습니다.",
    ...
    sections: [
      {
        ...
        children: [
          ...
          bodyParagraph(
            "환경변수를 하나도 설정하지 않아도 애플리케이션은 실행됩니다. 이 '데모 모드'에서 19개 대시보드 화면의 모든 수치는 내장 샘플 데이터를 실제 계산 엔진으로 계산한 값이며, 저장(쓰기) 동작만 거부됩니다. 즉, 지금 화면에 보이는 숫자는 하드코딩된 값이 아니라 계산된 값이고, 데이터베이스를 연결하는 즉시 같은 엔진이 실제 데이터를 계산합니다.",
          ),
